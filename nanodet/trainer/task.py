...
class TrainingTask(LightningModule):
    ...
    def training_epoch_end(self, outputs: List[Any]) -> None:
        self.trainer.save_checkpoint(os.path.join(self.logger.model_dir, "epoch_%d.ckpt" % (self.current_epoch)))
    ...
    def validation_epoch_end(self, validation_step_outputs):
        ...
        if all_results:
            eval_results = self.evaluator.evaluate(
                all_results, self.logger.logs_dir, rank=self.local_rank
            )
            metric = eval_results[self.cfg.evaluator.save_key]
            # save best model
            if metric > self.save_flag:
                self.save_flag = metric
                best_save_path = os.path.join(self.logger.logs_dir, "model_best")
                mkdir(self.local_rank, best_save_path)
                self.trainer.save_checkpoint(
                    os.path.join(best_save_path, "model_best.ckpt")
                )
                self.save_model_state(
                    os.path.join(best_save_path, "cat_room_09_03.pth")
                )
    ...
    def test_epoch_end(self, test_step_outputs):
        ...
        if all_results:
            ...
            json_path = os.path.join(self.logger.logs_dir, "results.json")
            ...
            if self.cfg.test_mode == "val":
                ...
                txt_path = os.path.join(self.logger.logs_dir, "eval_results.txt")
                with open(txt_path, "a") as f:
                    for k, v in eval_results.items():
                        f.write("{}: {}\n".format(k, v))
        else:
