...
class GFLHead(nn.Module):
    ...
    def post_process(self, preds, meta):
        print("gfl_head.py L.531 pred shape: ", preds.shape)
        cls_scores, bbox_preds = preds.split(
            [self.num_classes, 4 * (self.reg_max + 1)], dim=-1
        )
    ...
    def show_result(self, img, dets, class_names, score_thres=0.3, show=True, save_path=None):
        result, all_label, crop_img = overlay_bbox_cv(img, dets, class_names, score_thresh=score_thres)
        if show:
            cv2.imshow("det", result)
        return result, all_label, crop_img
